...
class DataStructure(BaseModel):
    ...
    s1: float = Field(..., description="tc (T-Cell count)", gt = 0)
    s2: float = Field(..., description="ldl cholesterol", gt = 0)
    s3: float = Field(..., description="hdl cholesterol", gt = 0)
    s4: float = Field(..., description="tch (thyroid hormone)", gt = 0)
    s5: float = Field(..., description="ltg (log of serum triglycerides)", gt = 0)
    s6: float = Field(..., description="glu (blood glucose)", gt = 0)
